...
export function PlateFrame({ label, status, children, className = "" }: PlateFrameProps) {
  return (
    <div className={`relative border border-[rgba(255,255,255,0.08)] ${className}`}>
      ...
      {status && (
        <span className="absolute top-4 right-4 z-10 font-mono-jb text-[9px] uppercase tracking-[0.22em] text-[#16C784]">
          {status}
        </span>
      )}
      {children}
      {label && (
        <span className="absolute bottom-4 left-4 z-10 font-mono-jb text-[9px] uppercase tracking-[0.22em] text-[#A6ADBB]">
          {label}
        </span>
      )}
    </div>
