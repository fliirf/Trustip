    ...
    <div role="alert" className="desk-row py-3 text-sm">
      <div className="flex flex-wrap items-start justify-between gap-x-6 gap-y-3">
        <div className="min-w-0 flex-1">
          <div className="micro-label text-blood">{title ?? d.ui.seller.rejected}</div>
...
function ErrorPage({
  ...
  const d = useDict();
  const heading = title ?? d.ui.genericErrorTitle;
  ...
  if (surface === "status") {
    ...
    return (
      <main className="mx-auto grid min-h-[100dvh] max-w-4xl items-center gap-10 px-6 py-16 lg:grid-cols-[240px_1fr] lg:gap-16">
        ...
        <div>
          <div className="micro-label text-ash">{d.status.eyebrow}</div>
...
export function ProtocolState({
  ...
  if (surface === "checkout") {
    return (
      <div role="status" className="terminal-panel px-5 py-4">
        <div className="micro-label text-ash">{d.ui.checkout.initializingPayment}</div>
  ...
  if (surface === "status") {
    return (
      <div role="status" className="relative pl-6">
        ...
        />
        <div className="micro-label text-ash">{d.ui.status.telemetryLabel}</div>
  ...
  return (
    <div role="status" className="desk-sheet px-1 py-2">
      <div className="micro-label text-ash">{d.ui.seller.operationQueue}</div>
      <div className="mt-2 text-sm font-medium text-mist">{label}</div>
      {detail && <div className="micro-label mt-1 text-ash">{detail}</div>}
  ...
  if (surface === "checkout") {
    ...
    return (
      <div className="terminal-panel flex flex-col items-start px-8 py-10">
        ...
        <div className="micro-label mt-8 text-ash">{d.ui.checkout.waitingTerminal}</div>
  ...
  if (surface === "status") {
    ...
    return (
      <div className="relative grid items-center gap-8 pl-6 lg:grid-cols-[200px_1fr] lg:gap-12">
        ...
        <div>
          <div className="micro-label text-ash">{d.ui.status.noTelemetryLabel}</div>
          <p className="os-reading mt-3 text-bone">{title}</p>
  ...
  return (
    <div className="desk-sheet max-w-2xl px-6 py-10">
      <div className="flex flex-wrap items-start gap-8">
        ...
        <div className="min-w-0 flex-1">
          <div className="micro-label text-ash">{d.ui.seller.emptyDesk}</div>
          <p className="os-reading mt-3 text-bone">{title}</p>
          <p className="mt-2 max-w-[42ch] os-body text-mist/70">{detail}</p>
          {action && (
            <div className="mt-7">
